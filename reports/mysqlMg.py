    ...
    def __getConn():
        ...
        if Mysql.__pool is None:
            __pool = PooledDB(creator=MySQLdb, mincached=1 , maxcached=20 ,
                              ...
                              db='url_detect',use_unicode=False,charset='utf8',cursorclass=DictCursor)
...
if __name__ == '__main__':
    mysql = Mysql()
    mysql.begin()
    insert = "insert into blacklist(keyword) values(%s) "
    params = ('黑产')
    mysql.insertOne(insert, params)
    select = "select * from whitelist limit 100"
    params = ()
    result =  mysql.getOne(select,params)
    for item in result:
        print item
    mysql.dispose()